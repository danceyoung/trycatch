 ...
 */
public class FilterHttpSink extends AbstractSink implements Configurable {
...
//    private static final Logger LOG = LogManager.getLogger(HttpSink.class);
    ...
    public final void configure(final Context context) {
        ...
        System.out.println("Read endpoint URL from configuration : " + configuredEndpoint);
        ...
        System.out.println("Using connect timeout : " + connectTimeout);
        ...
        System.out.println("Using request timeout : " + requestTimeout);

        acceptHeader = context.getString("acceptHeader", DEFAULT_ACCEPT_HEADER);
        System.out.println("Using Accept header value : " + acceptHeader);

        contentTypeHeader = context.getString("contentTypeHeader",
                DEFAULT_CONTENT_TYPE);
        System.out.println("Using Content-Type header value : " + contentTypeHeader);

        defaultBackoff = context.getBoolean("defaultBackoff", true);
        System.out.println("Channel backoff by default is " + defaultBackoff);

        defaultRollback = context.getBoolean("defaultRollback", true);
        System.out.println("Transaction rollback by default is " + defaultRollback);

        defaultIncrementMetrics = context.getBoolean("defaultIncrementMetrics",
                false);
        System.out.println("Incrementing metrics by default is " + defaultIncrementMetrics);
    ...
    public final void start() {
        System.out.println("Starting HttpSink");
        sinkCounter.start();
    }

    @Override
    public final void stop() {
        System.out.println("Stopping HttpSink");
        sinkCounter.stop();
    }
    ...
    public final Status process() throws EventDeliveryException {
        ...
        try {
            ...
            if (eventBody != null && eventBody.length > 0) {
                String allBodyString = new String(event.getBody());

                boolean isOnlyIncludes = allBodyString.contains(onlyIncludesContent);
                if (isOnlyIncludes) {
                    ...
                    try {
                        ...
                        System.out.println("Got status code : " + httpStatusCode);
                        ...
                        System.out.println("Response processed and closed");
                        ...
                        if (httpStatusCode >= HTTP_STATUS_CONTINUE) {
                            ...
                            if (shouldRollback) {
                                if (shouldBackoff) {
                                    System.out.println(String.format("Got status code %d from HTTP server."
                            ...
                            System.out.println("Malformed response returned from server, retrying");
                        }
                        ...
                        System.out.println("Error opening connection, or request timed out " + e);
                    }
                } else {
                    txn.commit();
                    status = Status.BACKOFF;

                    System.out.println("Processed content event not includes specified content");
                }
                ...
                System.out.println("FilterHttpSink Processed empty event");
            }
            ...
            System.out.println("Error sending HTTP request, retrying " + t);
    ...
    private void parseConfigOverrides(final String propertyName,
        ...
        if (config != null) {
            for (Map.Entry<String, String> value : config.entrySet()) {
                System.out.println(String.format("Read %s value for status code %s as %s",
                        propertyName, value.getKey(), value.getValue()));

                if (override.containsKey(value.getKey())) {
                    System.out.println(String.format("Ignoring duplicate config value for %s.%s",
